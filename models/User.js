...
const userSchema = new mongoose.Schema(
  {
    ...
    email: {
      ...
      lowercase: true,
      trim:      true,
      // email is optional — no unique/required constraints.
      // The sparse index below (userSchema.index) handles null deduplication.
      // NOTE: do NOT add sparse:true here — that would duplicate the index
      // declared at the bottom and trigger a Mongoose duplicate-index warning.
    ...
    phone: {
      ...
      trim:     true,
      // FIX: Removed unique: false — it is the default and caused a duplicate index
      // alongside userSchema.index({ phone: 1 }) below, triggering syncIndexes warnings.
      // Up to 3 accounts per phone number is enforced in route logic only.
