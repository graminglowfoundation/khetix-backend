...
router.get('/profile', async (req, res, next) => {
  try {
    ...
    if (userObj.farmLocation && userObj.farmLocation.coordinates && userObj.farmLocation.coordinates.length === 2) {
      // MongoDB stores as [longitude, latitude]
      userObj.longitude = userObj.farmLocation.coordinates[0];
      userObj.latitude = userObj.farmLocation.coordinates[1];
      // Hide the raw 0,0 default if GPS hasn't been saved yet
      if (userObj.longitude === 0 && userObj.latitude === 0) {
          userObj.longitude = null;
          userObj.latitude = null;
      }
    }
...
router.put('/profile', validate(schemas.updateProfile), async (req, res, next) => {
  try {
    // NOTE: 'preferences' is intentionally excluded from ALLOWED here.
    // Preferences are merged at the field level below to prevent wiping
    // alertSettings or other nested keys the client didn't send.
    const ALLOWED = ['name', 'email', 'phone', 'village', 'district', 'state', 'farmLocation', 'landAcres', 'farmType', 'crops', 'latitude', 'longitude'];
    const updates = {};
    const unsets  = {}; // FIX Bug 2: declare unsets so farmType/email clearing doesn't crash
    ALLOWED.forEach((k) => { if (req.body[k] !== undefined) updates[k] = req.body[k]; });

    // 🔥 SAFE ENUM HANDLING: Removes farmType cleanly instead of throwing Mongoose enum errors
    if (req.body.farmType !== undefined) {
        if (req.body.farmType === '' || req.body.farmType === null){
          unsets.farmType = 1;
          delete updates.farmType;
        }
        else updates.farmType = req.body.farmType;
    }

    // 🔥 SAFE EMAIL HANDLING: Safely clear or update email
    if (req.body.email !== undefined) {
        if (req.body.email === '' || req.body.email === null) {
            ...
            const emailCount = await User.countDocuments({ email: targetEmail, _id: { $ne: req.user.id } });
            if (emailCount >= 3) return res.status(409).json({ success: false, message: 'Maximum 3 accounts can share this email.' });
    ...
    const currentUser = await User.findById(req.user.id).select('name phone preferences');
    // 🔥 PREVENT ACCIDENTAL WIPE: Ignore empty strings for required fields if frontend sends them by mistake
    if (updates.name === '' || updates.name === null) delete updates.name;
    if (updates.phone === '' || updates.phone === null) delete updates.phone;
    ...
    // Convert flat latitude/longitude into MongoDB GeoJSON before saving
    if (updates.latitude !== undefined && updates.longitude !== undefined) {
      if (updates.latitude !== null && updates.longitude !== null) {
        updates.farmLocation = {
          type: 'Point',
          coordinates: [updates.longitude, updates.latitude] // Longitude ALWAYS goes first in GeoJSON
        };
      }
    ...
    // ── FIX Bug 1: Merge preferences at the key level so alertSettings
    //    (and other nested keys) are NEVER wiped by a partial update.
    //    e.g. sending { preferences: { language: 'hi' } } must NOT delete
    //    preferences.alertSettings or preferences.darkMode.
    if (req.body.preferences !== undefined && req.body.preferences !== null) {
      const incoming = req.body.preferences;
      const existing = currentUser.preferences?.toObject?.() ?? currentUser.preferences ?? {};

      // Top-level preference keys (language, darkMode, textSize, notifications)
      for (const [key, val] of Object.entries(incoming)) {
        if (key === 'alertSettings') {
          // Deep-merge alertSettings so individual alert flags are preserved
          updates[`preferences.alertSettings`] = {
            ...(existing.alertSettings ?? {}),
            ...val,
          };
        } else if (key === 'notifications' && typeof val === 'object') {
          // Merge notification toggles (push/email/sms) without wiping others
          updates[`preferences.notifications`] = {
            ...(existing.notifications ?? {}),
            ...val,
          };
        } else {
          // Scalar preference (language, darkMode, textSize) — safe to set directly
          updates[`preferences.${key}`] = val;
        }
      }
    }

    // Build the final MongoDB operation
    const mongoOp = { $set: updates };
    if (Object.keys(unsets).length > 0) mongoOp.$unset = unsets;

    const user = await User.findByIdAndUpdate(req.user.id, mongoOp, { new: true, runValidators: true });
    if (!user) return res.status(404).json({ success: false, message: 'User not found.' });
...
router.post('/change-password', validate(schemas.changePassword), async (req, res, next) => {
  try {
    ...
    user.plainPassword = newPassword; // FIX: keep plainPassword in sync so forgot-password returns the latest password
    user.refreshTokens = [];
    await user.save();
...
// FIX: Read password from req.body.password (what Android sends via request body).
//      Also accepts X-Confirm-Password header as fallback for other clients.
router.delete('/account', validate(schemas.deleteAccount), async (req, res, next) => {
  try {
    // FIX: Android sends password in body → read from req.body.password first,
    ...
    // FIX: Use findByIdAndUpdate instead of save() to avoid pre-save hooks running
    //      on the deliberately invalid "deleted_xxx" phone value.
    //      Set email to null so it's cleanly removed from the sparse unique index.
    await User.findByIdAndUpdate(
      req.user.id,
      {
        $set: {
          isActive: false,
          email:    null,                  // null = dropped from sparse unique index
          phone:    `deleted_${user._id}`, // unique per user, avoids phone conflicts
          fcmToken: '',
        },
      },
      { runValidators: false }
