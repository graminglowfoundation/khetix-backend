...
export const schemas = {
  ...
  updateProfile: Joi.object({
    ...
    email: Joi.string().email({ tlds: { allow: false } }).lowercase().trim().optional().allow('').messages({
      'string.email': 'Please provide a valid email address.',
    }),
    phone: Joi.string().pattern(/^(\+91[\s-]?)?[6-9]\d{9}$/).optional().allow(''),
    village: Joi.string().max(100).trim().allow('').optional(),
    district: Joi.string().max(100).trim().allow('').optional(),
    state: Joi.string().max(100).trim().allow('').optional(),
